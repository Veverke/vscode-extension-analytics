...
import { computeMonthlyRollup, writeMonthlyRollup, readMonthlyRollup } from '../monthly.js'
...
describe('computeMonthlyRollup', () => {
  ...
  it('handles negative installsGained by clamping to 0', () => {
    const data = [
      makePoint('2026-05-20T12:00:00Z', 1000),
      makePoint('2026-06-01T00:00:00Z', 500),
    ]
    const result = computeMonthlyRollup(data)
    expect(result[1].installsGained).toBe(0) // clamped to 0
  })

  it('captures github stats from last point in month', () => {
    const data: DataPoint[] = [
      {
        ts: '2026-05-20T12:00:00Z',
        marketplace: { installs: 500, updates: 0, averageRating: 4.0, ratingCount: 1, trendingWeekly: 0, trendingMonthly: 0 },
        openVsx: null,
        github: { stars: 10, forks: 3, contributions: 5 },
      },
    ]
    const result = computeMonthlyRollup(data)
    expect(result[0].starsEndOfMonth).toBe(10)
    expect(result[0].forksEndOfMonth).toBe(3)
    expect(result[0].contributionsEndOfMonth).toBe(5)
  })

  it('handles null github stats with defaults', () => {
    const data: DataPoint[] = [
      {
        ts: '2026-05-20T12:00:00Z',
        marketplace: { installs: 500, updates: 0, averageRating: 4.0, ratingCount: 1, trendingWeekly: 0, trendingMonthly: 0 },
    ...
    const result = computeMonthlyRollup(data)
    expect(result[0].starsEndOfMonth).toBe(0)
    expect(result[0].forksEndOfMonth).toBe(0)
    expect(result[0].contributionsEndOfMonth).toBe(0)
  })
})

describe('writeMonthlyRollup and readMonthlyRollup', () => {
  let tmpDir: string

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'monthly-test-'))
    setDataDir(tmpDir)
  })
  ...
  it('writeMonthlyRollup writes file and readMonthlyRollup reads it back', () => {
    const rollups = [
      { yearMonth: '2026-05', installsEndOfMonth: 1000, installsGained: 0, avgRating: 4.5, ratingCountEndOfMonth: 10, openVsxDownloadsEndOfMonth: 100, dataPointsInMonth: 5, starsEndOfMonth: 10, forksEndOfMonth: 3, contributionsEndOfMonth: 1 },
    ]
    writeMonthlyRollup('test.ext', rollups)
    const result = readMonthlyRollup('test.ext')
    expect(result).toEqual(rollups)
  })

  it('readMonthlyRollup returns empty array when file does not exist', () => {
    const result = readMonthlyRollup('nonexistent.ext')
